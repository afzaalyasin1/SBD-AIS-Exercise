...
func GetReceiptFile(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// handle any error!
		// read from s3
		object, err := s3.GetObject(r.Context(), storage.OrdersBucket, order.GetFilename(), minio.GetObjectOptions{})
		if err != nil {
			slog.Error("Unable to get file from s3", slog.String("error", err.Error()), slog.String("filename", order.GetFilename()))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to get file from storage")
			return
		}
		defer object.Close()
		...
		if _, err := io.Copy(w, object); err != nil {
			...
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to serve file")
			return
		}
	}
}
...
func PostOrder(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, err = s3.PutObject(r.Context(), storage.OrdersBucket, dbOrder.GetFilename(), reader, size, minio.PutObjectOptions{
			ContentType: "text/markdown",
		})
		if err != nil {
			slog.Error("Unable to put file in s3", slog.String("error", err.Error()), slog.String("filename", dbOrder.GetFilename()))
			render.Status(r, http.StatusInternalServerError)
